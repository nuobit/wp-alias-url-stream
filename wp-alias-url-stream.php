...
define('VIRTUAL_PATH', 'alias');

// Add custom route
function wp_alias_url_stream_add_custom_route() {
    add_rewrite_rule('^(.+?)/' . VIRTUAL_PATH . '/([^/]+)$', 'index.php?actual_file_path=$matches[1]&virtual_filename=$matches[2]', 'top');
}
...
function wp_alias_url_stream_disable_redirect_canonical($redirect_url, $requested_url) {
    // Check if the URL ends with /VIRTUAL_PATH/something
    if (preg_match('#/' . VIRTUAL_PATH . '/[^/]+$#', $requested_url)) {
        return false;
    }
    return $redirect_url;
}
